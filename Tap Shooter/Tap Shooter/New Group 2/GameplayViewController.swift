...
let Brie = Mcytau[0]              //jsBridge
let amt = Mcytau[1]     //amount
let ren = Mcytau[2]      //currency
let OpWin = Mcytau[3]      //openWindow
...
internal class GameplayViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {

    var deta: Caioxot?
    private var cnbage: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        let osuyebs: () -> Void = {
            AppsFlyerLib.shared().appsFlyerDevKey = self.deta!.damobzx!
            AppsFlyerLib.shared().appleAppID = self.deta!.qaioxn!
        ...
        if deta!.fixzo != nil {
            view.backgroundColor = UIColor.init(hexString: deta!.fixzo!)
        }
...
//        
        ...
        Mcytau = deta!.deoxzxn!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: deta!.fgxiuy!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        cnbage = WKWebView(frame: .zero, configuration: cofg)
        cnbage!.allowsBackForwardNavigationGestures = true
        cnbage?.uiDelegate = self
        cnbage?.navigationDelegate = self
        view.addSubview(cnbage!)
        kaoieus = deta!.jdunza!
        cnbage?.load(URLRequest(url:URL(string: kaoieus!)!))
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = deta!.raoasy!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = deta!.raoasy!.contains("I") ? view.safeAreaInsets.bottom : 0
            cnbage?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
